...
Para trabajar sin internet, inicia sesión y abre **Jornada** con conexión una vez en ese dispositivo. La aplicación conserva los clientes y los registros en el almacenamiento local del navegador, separados por cuenta. Se puede reabrir sin señal usando la última sesión local; enviar datos siempre requiere una sesión válida en el servidor. La recuperación de conexión y un reintento automático cada 30 segundos mientras Avanza permanece abierta procesan la cola. Cada operación tiene un identificador estable para evitar duplicados si se pierde la respuesta. Si la sesión vence, hay que iniciar sesión de nuevo; los registros pendientes se conservan. No borres los datos del navegador mientras existan pendientes.
...
## Clientes compartidos y ajustes de interfaz

Todos los módulos usan la colección canónica companies, con un mismo identificador por cliente. La migración conserva las visitas, chats y coordenadas existentes y guarda una copia previa en data/database.before-client-unification.json. Los clientes creados en Jornada aparecen también en Clientes; el administrador puede completar su ficha y activar su chat seleccionando al menos dos integrantes internos. No se crean cuentas de acceso automáticamente para clientes de campo.

Jornada agrupa los controles por función y sincroniza automáticamente, sin botón manual. En Chat móvil el menú inferior se oculta y Atrás vuelve a la pantalla anterior. Las tareas muestran 25 % amarillo, 50 % azul, 75 % verde y finalizadas en rojo tachado. Las acciones de tareas ya no generan mensajes de chat; las notificaciones móviles de tareas se conservan. Eliminar una tarea requiere su botón explícito y una confirmación identificada que se puede cancelar; se corrigió el enlace accidental de eventos de Maquinarias a filas de Tareas.

Productos permite al cliente combinar la búsqueda con una división (ALM, AGR, IMP, PAP, MAT o MED), manteniendo las cantidades seleccionadas. La tabla usa anchos 40/9/10/11 para Producto, Cantidad, Precio e Importe. Los artículos sin precio admiten cantidades y envío; se muestran por confirmar y se excluyen del subtotal con precio, también en el Excel editable. El responsable asignado puede eliminar sus propias tareas de maquinaria, con confirmación; otros empleados no. Chat en PC aprovecha la altura disponible con el compositor separado del historial desplazable.