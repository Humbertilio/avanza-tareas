...
function showView(name, addHistory = true) { if(['tracking','jornada'].includes(name))name='visits'; window.cancelTaskDeletion?.(); $('#appView').classList.toggle('chat-mode',name==='chat'); ['tasks','all','new','machines','visits','inventory','products','chat','clients','users'].forEach(x => $(`#${x}View`).classList.toggle('hidden', x !== name)); $$('.nav').forEach(x => x.classList.toggle('active', x.dataset.view === name)); $('.mobile-new').classList.toggle('hidden', ['machines','visits','inventory','products','chat','clients','users'].includes(name)); $('#greeting').textContent = name === 'tasks' ? 'Tus tareas' : name === 'all' ? 'Todas las tareas' : name === 'new' ? 'Crear una tarea' : name === 'machines' ? 'Maquinarias' : name === 'visits' ? 'Jornada' : name === 'inventory' ? 'Inventario' : name === 'products' ? 'Productos' : name === 'chat' ? 'Chat' : name==='clients'?'Clientes':'Gestión del equipo'; const sameScreen=history.state?.view===name&&!history.state?.machineId&&!history.state?.conversationId;if(addHistory&&!sameScreen)history.pushState({view:name},'',`#${name}`);if(name==='machines')window.loadMachinery?.();if(name==='visits')window.loadVisits?.();if(name==='inventory')window.loadInventory?.();if(name==='products')window.loadProducts?.();if(name==='chat')window.loadChat?.();if(name==='clients')window.loadClients?.(); }
...
window.addEventListener('popstate', event => { if (!me) return; const state = event.state || { view: 'tasks' }; showView(state.view || 'tasks', false); if (state.view === 'machines') setTimeout(() => window.openMachineFromHistory?.(state.machineId || null), 0); if(state.view==='chat')setTimeout(()=>window.openChatFromHistory?.(state.conversationId||null),0); });
boot();
