...
function taskRow(task) {
  ...
  const canNote=task.creatorId===me.id||task.assigneeId===me.id,actions=`${task.creatorId===me.id?`<button class="edit-task" data-id="${task.id}">Editar</button>`:''}${canNote?`<button class="add-note" data-id="${task.id}">Anotar</button>`:''}`||'—';
  ...
  const description=`${task.description?`<div class="original-description">${escapeHtml(task.description)}</div>`:'<span class="no-description">Sin descripción inicial</span>'}${notes}`;
  return `<tr class="${task.progress===100?'completed-row':''}"><td><strong>${escapeHtml(task.title)}</strong></td><td class="description-cell">${description}</td><td>${dateText(task.createdAt)}</td><td>${escapeHtml(task.creatorName)}</td><td>${escapeHtml(task.assigneeName)}</td><td>${dueDateText(task.dueDate)}</td><td>${statusControl}</td><td>${reading}</td><td class="row-actions">${actions}</td></tr>`;
}
