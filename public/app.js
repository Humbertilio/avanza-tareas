...
async function enterApp() {
  ...
  if(me.role==='client'){$$('.employee-nav').forEach(node=>node.classList.add('hidden'));const clientView=location.hash==='#inventory'?'inventory':'chat';history.replaceState({view:clientView},'',`#${clientView}`);showView(clientView,false);return;}
...
function showView(name, addHistory = true) { ['tasks','all','new','machines','inventory','chat','clients','users'].forEach(x => $(`#${x}View`).classList.toggle('hidden', x !== name)); $$('.nav').forEach(x => x.classList.toggle('active', x.dataset.view === name)); $('.mobile-new').classList.toggle('hidden', ['machines','inventory','chat','clients','users'].includes(name)); $('#greeting').textContent = name === 'tasks' ? 'Tus tareas' : name === 'all' ? 'Todas las tareas' : name === 'new' ? 'Crear una tarea' : name === 'machines' ? 'Maquinarias' : name === 'inventory' ? 'Inventario' : name === 'chat' ? 'Chat' : name==='clients'?'Clientes':'Gestión del equipo'; const sameScreen=history.state?.view===name&&!history.state?.machineId&&!history.state?.conversationId;if(addHistory&&!sameScreen)history.pushState({view:name},'',`#${name}`);if(name==='machines')window.loadMachinery?.();if(name==='inventory')window.loadInventory?.();if(name==='chat')window.loadChat?.();if(name==='clients')window.loadClients?.(); }
