...
function renderTasks() {
  ...
  $$('#taskList tr[data-task-id],#allTaskList tr[data-task-id]').forEach(row=>{row.addEventListener('click',event=>{if(event.target.closest('button,select,input,details,.description-cell'))return;const task=tasks.find(item=>item.id===row.dataset.taskId);if(task&&(task.creatorId===me.id||me.role==='admin'))openEditTask(task.id);});row.addEventListener('dblclick',event=>{if(me.role!=='admin'||event.target.closest('button,select,input,details'))return;event.preventDefault();deleteTask(row.dataset.taskId);});});
...
function taskRow(task) {
  ...
  return `<tr data-task-id="${task.id}" class="${task.progress===100?'completed-row':''}"><td>${canEdit?`<button class="task-title-button" data-id="${task.id}" title="Modificar tarea">${escapeHtml(task.title)}</button>`:`<strong>${escapeHtml(task.title)}</strong>`}</td><td class="description-cell ${canNote?'can-comment':''}" data-task-id="${task.id}" tabindex="0" role="button" title="Doble clic para ver todo">${description}</td><td>${dateText(task.createdAt)}</td><td>${escapeHtml(task.creatorName)}</td><td>${escapeHtml(task.assigneeName)}</td><td>${dueDateText(task.dueDate)}</td><td>${statusControl}</td><td>${reading}</td><td class="row-actions">${actions}</td></tr>`;
}
