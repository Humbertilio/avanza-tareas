...
async function enterApp(offline = false) {
  ...
  if (['admin','seller'].includes(me.role)) { $('.visits-nav').classList.remove('hidden'); }
  if (offline) { showView('visits', false); return; }
  if(me.role==='client'){$$('.employee-nav').forEach(node=>node.classList.add('hidden'));const clientView=['#inventory','#products'].includes(location.hash)?location.hash.slice(1):'chat';if(clientView!=='chat')history.replaceState({view:clientView},'',`#${clientView}`);showView(clientView,false);return;}
  await refresh(); if(location.hash==='#inventory'){history.replaceState({view:'tasks'},'','#tasks');history.pushState({view:'inventory'},'','#inventory');showView('inventory',false);return;} if(location.hash.startsWith('#chat')){showView('chat',false);return;} history.replaceState({ view: 'tasks' }, '', '#tasks'); showView('tasks', false);
}
...
function renderTasks() {
  ...
  $$('#taskList tr[data-task-id],#allTaskList tr[data-task-id]').forEach(row=>{row.addEventListener('click',event=>{if(event.target.closest('button,select,input,details,.description-cell'))return;const task=tasks.find(item=>item.id===row.dataset.taskId);if(task&&(task.creatorId===me.id||me.role==='admin'))openEditTask(task.id);});});
...
function taskRow(task) {
  ...
  return `<tr data-task-id="${task.id}" class="task-progress-${task.progress} ${task.progress===100?'completed-row':''}"><td>${canEdit?`<button class="task-title-button" data-id="${task.id}" title="Modificar tarea">${escapeHtml(task.title)}</button>`:`<strong>${escapeHtml(task.title)}</strong>`}</td><td class="description-cell ${canNote?'can-comment':''}" data-task-id="${task.id}" tabindex="0" role="button" title="Doble clic para ver todo">${description}</td><td>${dateText(task.createdAt)}</td><td>${escapeHtml(task.creatorName)}</td><td>${escapeHtml(task.assigneeName)}</td><td>${dueDateText(task.dueDate)}</td><td>${statusControl}</td><td>${reading}</td><td class="row-actions">${actions}</td></tr>`;
}
function renderUsers() { $('#userList').innerHTML = users.filter(user=>user.role!=='client').map(user => `<div class="user-item ${user.active?'':'inactive'}"><div><strong>${escapeHtml(user.name)}</strong><span>@${escapeHtml(user.username)} · ${user.role === 'admin' ? 'Administrador' : user.role==='seller'?'Vendedor':'Empleado'} · ${user.active?'Activo':'Desactivado'}</span></div>${me.role==='admin'?`<button class="edit-user" data-id="${user.id}">Editar</button>`:''}</div>`).join(''); $$('.edit-user').forEach(button=>button.addEventListener('click',()=>openEditUser(button.dataset.id))); }
...
async function deleteTask(id) {
  ...
  if(deletingTask || !task || me.role!=='admin')return;
  deletingTask=true;
  try {if(!await window.confirmTaskDeletion(task.title))return;await request(`/api/tasks/${id}`,{method:'DELETE'});await refresh();toast('Tarea eliminada');}
  catch(error){toast(error.message);}finally{deletingTask=false;}
}
const chatHomeButton=document.createElement('button');
chatHomeButton.id='chatHome';chatHomeButton.type='button';chatHomeButton.textContent='‹ Inicio';chatHomeButton.title='Volver al inicio de Avanza';chatHomeButton.className='hidden';
$('#greeting').before(chatHomeButton);
const homeView=()=>me?.role==='client'?'products':'tasks';
window.exitAvanzaChat=()=>{window.openChatFromHistory?.(null);history.replaceState({view:homeView()},'',`#${homeView()}`);showView(homeView(),false);};
chatHomeButton.addEventListener('click',()=>window.exitAvanzaChat());
function showView(name, addHistory = true) { window.setInventoryActive?.(name==='inventory'); if(name==='chat'&&!history.state?.chatEntry){const conversationId=location.hash.startsWith('#chat/')?location.hash.slice(6):null;if(!addHistory)history.replaceState({view:homeView()},'',`#${homeView()}`);history.pushState({view:'chat',chatEntry:true,conversationId},'',conversationId?`#chat/${conversationId}`:'#chat');addHistory=false;}chatHomeButton.classList.toggle('hidden',name!=='chat'); if(['tracking','jornada'].includes(name))name='visits'; window.cancelTaskDeletion?.(); $('#appView').classList.toggle('chat-mode',name==='chat'); ['tasks','all','new','machines','visits','inventory','products','chat','clients','users'].forEach(x => $(`#${x}View`).classList.toggle('hidden', x !== name)); $$('.nav').forEach(x => x.classList.toggle('active', x.dataset.view === name)); $('.mobile-new').classList.toggle('hidden', ['machines','visits','inventory','products','chat','clients','users'].includes(name)); $('#greeting').textContent = name === 'tasks' ? 'Tus tareas' : name === 'all' ? 'Todas las tareas' : name === 'new' ? 'Crear una tarea' : name === 'machines' ? 'Maquinarias' : name === 'visits' ? 'Jornada' : name === 'inventory' ? 'Inventario' : name === 'products' ? 'Productos' : name === 'chat' ? 'Chat' : name==='clients'?'Clientes':'Gestión del equipo'; const sameScreen=history.state?.view===name&&!history.state?.machineId&&(name==='chat'||!history.state?.conversationId);if(addHistory&&!sameScreen)history.pushState({view:name},'',`#${name}`);if(name==='machines')window.loadMachinery?.();if(name==='visits')window.loadVisits?.();if(name==='inventory')window.loadInventory?.();if(name==='products')window.loadProducts?.();if(name==='chat')window.loadChat?.();if(name==='clients')window.loadClients?.(); }
...
const closeClientRequest=()=>$('#clientRequestModal').classList.add('hidden');
$('#requestClientAccess').addEventListener('click',()=>{$('#clientRequestForm').reset();$('#clientRequestForm .formMessage').textContent='';$('#clientRequestModal').classList.remove('hidden');});
$('#closeClientRequest').addEventListener('click',closeClientRequest);$('#cancelClientRequest').addEventListener('click',closeClientRequest);$('#clientRequestModal').addEventListener('click',event=>{if(event.target===$('#clientRequestModal'))closeClientRequest();});
$$('#clientRequestForm input[name="companyPhone"],#clientRequestForm input[name="phone"]').forEach(input=>input.addEventListener('input',()=>{const digits=input.value.replace(/\D/g,'').slice(0,10);input.value=[digits.slice(0,3),digits.slice(3,6),digits.slice(6,10)].filter(Boolean).join('-');}));
...
document.addEventListener('keydown',event=>{if(event.key!=='Escape')return;const modal=$$('.modal:not(.hidden)').at(-1);if(modal){modal.classList.add('hidden');return;}if(history.state?.machineId||history.state?.conversationId)history.back();});
window.addEventListener('popstate', event => { if (!me) return; if($('#appView').classList.contains('chat-mode')){window.exitAvanzaChat();return;} const state = event.state || { view: 'tasks' }; showView(state.view || 'tasks', false); if (state.view === 'machines') setTimeout(() => window.openMachineFromHistory?.(state.machineId || null), 0); if(state.view==='chat')setTimeout(()=>window.openChatFromHistory?.(state.conversationId||null),0); });
boot();
