...
function renderTasks() {
  ...
  $('#taskList').innerHTML = `<table class="admin-table excel-table"><thead><tr>${taskHeaders()}</tr></thead><tbody>${shown.length ? shown.map(taskRow).join('') : '<tr><td colspan="9"><div class="empty">No hay tareas que coincidan con los filtros.</div></td></tr>'}</tbody></table>`;
  ...
  $$('.description-cell.can-comment').forEach(cell=>{cell.addEventListener('click',()=>openNote(cell.dataset.noteId));cell.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openNote(cell.dataset.noteId);}});});
...
function taskRow(task) {
  ...
  const notes=(task.notes||[]).map(note=>`<div class="note-entry"><span>${escapeHtml(note.text)}</span><small>${dateTimeText(note.createdAt)}</small></div>`).join('');
  const description=`${task.description?`<div class="original-description">${escapeHtml(task.description)}</div>`:'<span class="no-description">Sin descripción inicial</span>'}${notes}${canNote?'<div class="comment-hint">＋ Agregar comentario</div>':''}`;
  return `<tr class="${task.progress===100?'completed-row':''}"><td><strong>${escapeHtml(task.title)}</strong></td><td class="description-cell ${canNote?'can-comment':''}" ${canNote?`data-note-id="${task.id}" tabindex="0" role="button" title="Agregar comentario"`:''}>${description}</td><td>${dateText(task.createdAt)}</td><td>${escapeHtml(task.creatorName)}</td><td>${escapeHtml(task.assigneeName)}</td><td>${dueDateText(task.dueDate)}</td><td>${statusControl}</td><td>${reading}</td><td class="row-actions">${actions}</td></tr>`;
}
