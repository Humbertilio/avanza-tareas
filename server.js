...
async function api(req, res, url) {
  ...
  if(req.method==='DELETE'&&chatMessageMatch){
    try{
      const result=await mutateDb(db=>{const message=db.messages.find(item=>item.id===chatMessageMatch[1]&&!item.deletedAt);if(!message)throw Object.assign(new Error('Mensaje no encontrado'),{status:404});if(message.senderId!==user.id)throw Object.assign(new Error('Solo puede borrar sus propios mensajes'),{status:403});message.deletedAt=new Date().toISOString();message.updatedAt=message.deletedAt;db.attachments=(db.attachments||[]).filter(item=>item.messageId!==message.id);db.messageReceipts=(db.messageReceipts||[]).filter(item=>item.messageId!==message.id);const conversation=db.conversations.find(item=>item.id===message.conversationId);if(conversation)conversation.updatedAt=message.updatedAt;return{conversationId:message.conversationId,messageId:message.id};});
      const db=readDb();emitConversation(db,result.conversationId,'message-change',result);return json(res,200,{ok:true});
    }catch(error){return json(res,error.status||500,{error:error.message});}
  }
