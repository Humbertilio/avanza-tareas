...
function configurePush() {
  const db = readDb();
  let changed = false;
  if(db.meta.unifiedClients !== 1) {
    const backup=path.join(DATA_DIR,'database.before-client-unification.json');
    if(!fs.existsSync(backup))fs.copyFileSync(DB_FILE,backup,fs.constants.COPYFILE_EXCL);
    changed=require('./client-directory').migrateClients(db)||changed;
  }
...
async function notifyTaskEvent(sender, recipientId, task, status, module = 'tasks') {
  if(!recipientId || recipientId===sender.id)return;
  await notifyUser(recipientId,{title:'Tarea '+status,body:task.title,url:'/#'+module});
}
...
async function api(req, res, url) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/tasks') {
    ...
    try {
      ...
      await notifyTaskEvent(user,task.assigneeId,task,'asignada');
  ...
  if (req.method === 'PATCH' && taskMatch) {
    ...
    try {
      ...
      if(result.reassigned)await notifyTaskEvent(user,result.task.assigneeId,result.task,'reasignada');
  ...
  if (req.method === 'PATCH' && statusMatch) {
    ...
    try {
      ...
      if(result.justCompleted)await notifyTaskEvent(user,result.task.creatorId,result.task,'finalizada');
      return json(res, 200, { task: result.task });
    } catch (error) { return json(res, error.status || 500, { error: error.message }); }
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/machine-tasks') {
    ...
    try {
      ...
      await notifyTaskEvent(user,task.assigneeId,task,'asignada','machines');
      return json(res, 201, { task });
    } catch (error) { return json(res, error.status || 500, { error: error.message }); }
  }
  ...
  if (machineTaskAction && req.method === 'PATCH' && machineTaskAction[2] === 'status') {
    ...
    try { const result=await mutateDb(db=>{const found=(db.machineTasks||[]).find(item=>item.id===machineTaskAction[1]);if(!found)throw Object.assign(new Error('Tarea no encontrada'),{status:404});if(found.assigneeId!==user.id)throw Object.assign(new Error('Solo el responsable puede cambiar el estado'),{status:403});const justCompleted=found.progress!==100;found.progress=progress;found.updatedAt=new Date().toISOString();found.completedAt ||= found.updatedAt;return{task:found,justCompleted};});if(result.justCompleted)await notifyTaskEvent(user,result.task.creatorId,result.task,'finalizada','machines');return json(res,200,{task:result.task}); }
    catch(error){return json(res,error.status||500,{error:error.message});}
  }
